...
module.exports = tobz = async (tobz, message) => {
    try {
        ...
        const ownerNumber = '6285329981485@c.us'
        ...
        // FUNCTION
                ...
                // MRHRTZ
                 if (chats.match("anjing") || chats.match("gblk") || chats.match("tolol") || chats.match("kntl")) {
                    if (!isGroupAdmins) {
                        ...
                        .then(() => {
                            tobz.sendText(from, `*「 ANTI BADWORD 」*\nKamu Telah Melanggar Deskripsi Group, maaf kamu di kick dari grup 🙁`)
                        }).catch(() => tobz.sendText(from, `Untung yoni Bukan Admin, Kalo Jadi Admin Udah Aku Kick Tuh! 😑`))
                    } else {
                ...
                if(body === '#mute' && isMuted(chatId) == true){
                    if(isGroupMsg) {
                        if (!isAdmin) return tobz.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh admin yoni!', id)
                ...
                if (body === '#unbanchat') {
                    if (!isOwner) return tobz.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh Owner yoni!', id)
        ...
        case '#banchat':
            ...
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner yoni!', id)
...
${desc}`)
        ...
        case '#owner':
        case '#creator':
            tobz.sendContact(chatId, `6285329981485@c.us`)
        ...
        case '#simi':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Admin yoni!', id) // Hanya Admin yang bisa mengaktifkan
            if (args.length === 1) return tobz.reply(from, 'Pilih enable atau disable!', id)
            if (args[1].toLowerCase() === 'enable') {
                simi_.push(chat.id)
...
🌐️ *URL*: ${url}`
        ...
        case '#ytmp4':
            ...
            break
        case '#play':
            if (!isAdmin) return tobz.reply(from, `Perintah ini hanya bisa di gunakan oleh Admin yoni!`, id)
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
        ...
        case '#nhentai': // SEARCH NHENTAI
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            tobz.reply(from, '*Fitur Ini Hanya Bisa Digunakan Oleh User VIP Yoni Chan,* Ketik #owner untuk mendapatkan fitur VIP Yoni Chan >//<', id)
          break
...
➸ *Wilayah:* +${region_name}`
        ...
        case '#omentionall':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Owner yoni', id)
        ...
        case '#ekickall':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya untuk Owner yoni', id)
        ...
        case '#okickall':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya untuk Admin yoni', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
        ...
        case '#oadd':
            ...
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin yoni', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
            try {
        ...
        case '#oleave':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin yoni', id)
            await tobz.sendText(from,'yoni DIPERINTAHKAN KELUAR OLEH OWNER!!').then(() => tobz.leaveGroup(groupId))
            break
        case '#leave':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin group', id)
        ...
        case '#getses':
            ...
            break
        case '#yoniadmin':
            let admn = `This is list of yoni Admin\nTotal : ${adminNumber.length}\n`
        ...
        case '#addadmin':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner !', id)
                ...
                tobz.reply(from, 'Success Menambahkan Admin yoni!', id)
                }
            break
        case '#deladmin':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner yoni!', id)
                ...
                fs.writeFileSync('./lib/database/admin.json', JSON.stringify(adminNumber))
                tobz.reply(from, 'Success Menghapus Admin yoni!', id)
            break
        case '#block':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner yoni!', id)
            for (let i = 0; i < mentionedJidList.length; i++) {
        ...
        case `#say`:
            ...
            tobz.reply(from, rondKoy, id)
            break
        case '#daftar':
                if (args.length === 1) return tobz.reply(from, 'Jika ingin daftar silahkan ketik\n*#daftar add 628xxxxx*\n*#daftar del 628xxxxx*', id)
                if (args[1] === 'add') {
                    daftar.push(args[2]+'@c.us')
                    fs.writeFileSync('./lib/database/daftar.json', JSON.stringify(daftar))
                    tobz.reply(from, `╭─────「 *𝙍𝙀𝙂𝙄𝙎𝙏𝙍𝘼𝙎𝙄* 」
│ *Registrasi Success*
│
│ Whatsapp User : ${args[2]}
│ Status : ACTIVE
│
╰──「 𝗣𝗼𝘄𝗲𝗿𝗲𝗱 𝗯𝘆 *𝙃𝙖𝙩𝙨𝙪𝙣𝙚 𝙈𝙞𝙠𝙪* 」`, id)
                } else
                if (args[1] === 'del') {
                    let xnxx = daftar.indexOf(args[2]+'@c.us')
                    daftar.splice(xnxx,1)
                    fs.writeFileSync('./lib/database/daftar.json', JSON.stringify(daftar))
                    tobz.reply(from, `╭─────「 *𝙐𝙉𝙍𝙀𝙂𝙄𝙎𝙏𝙍𝘼𝙎𝙄* 」
│ *UnRegistrasi Success*
│
│ *Whatsapp User* : ${args[2]}
│ *Status* : OFF
│
╰──「 𝗣𝗼𝘄𝗲𝗿𝗲𝗱 𝗯𝘆 *𝙃𝙖𝙩𝙨𝙪𝙣𝙚 𝙈𝙞𝙠𝙪* 」`)
                } 
            break
       case '#addsay':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (args.length == 1) return tobz.reply(from, `Kirim perintah #addsay [teks]\ncontoh #addsay anjay`, id)
            const says = body.slice(8) 
            say.push(says)
            fs.writeFileSync('./lib/database/say.json', JSON.stringify(say))
            tobz.reply(from, `Add ${says} sukses!`,id)
            break
        case `#delsay`:
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (args.length == 1) return tobz.reply(from, `Kirim perintah #delsay [teks], contoh #delsay anjay`, id)
            const sayso = body.slice(8) 
            let delsayso = say.indexOf(sayso)
            say.splice(delsayso, 1)
            fs.writeFileSync('./lib/database/say.json', JSON.stringify(say))
            tobz.reply(from, `Delete ${sayso} sukses!`,id)
            break
        case `#saylist`:
            ...
            await tobz.reply(from, saylisto, id)
            break
        case '#addkotor':
            if (!isGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (args.length == 1) return tobz.reply(from, `Kirim perintah #addkotor [teks]\ncontoh #addkotor anjay`, id)
            const kotors = body.slice(8) 
            kotor.push(kotors)
            fs.writeFileSync('./lib/database/kasar.json', JSON.stringify(kotor))
            tobz.reply(from, `Add ${kotors} ke dalam database sukses!`,id)
            break
        case `#listkotor`:
            let kotorlisto = `Random list kotor\nTotal : ${kotor.length}\n`
            for (let i of kotor) {
            kotorlisto += `☛ ${i}\n`
            }
            await tobz.reply(from, kotorlisto, id)
            break
        case `#delsay`:
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (args.length == 1) return tobz.reply(from, `Kirim perintah #delkotor [teks], contoh #delkotor anjay`, id)
            const kotorso = body.slice(8) 
            let delkotorso = kotor.indexOf(sayso)
            kotor.splice(delkotorso, 1)
        ...
        case '#antilinkgc':
            ...
            if (args[1].toLowerCase() === 'enable') {
                lenkgc.push(chat.id)
                fs.writeFileSync('./lib/database/linkgc.json', JSON.stringify(lenkgc))
                tobz.reply(from, 'Fitur anti link grup berhasil di aktifkan di group ini!', id)
            } else if (args[1].toLowerCase() === 'disable') {
                lenkgc.splice(chat.id, 1)
                fs.writeFileSync('./lib/database/linkgc.json', JSON.stringify(lenkgc))
                tobz.reply(from, 'Fitur anti linkgrup berhasil di nonaktifkan!', id)
            } else {
                tobz.reply(from, 'Pilih enable atau disable udin!', id)
            }
            break
...
const lenkgc = JSON.parse(fs.readFileSync('./lib/database/linkgc.json'));
        const usLenkgc = isGroupMsg ? lenkgc.includes(chat.id) : false
        const isLinkgc = (chatId) => {
            if (lenkgc.includes(chatId)) {
                return false
            } else {
                return true
            }
        }
        if (usLenkgc && body.includes('https://chat.whatsapp.com/') && !isOwner) {
            await tobz.reply(from, 'DISINI ANTI LINK GC GAN, AUTO KICK!\n\nanda akan dikeluarkan dalam 3 detik', id)
			await tobz.reply(from, '3...', id)
			await sleep(700)
			await tobz.reply(from, '2...', id)
			await sleep(700)
			await tobz.reply(from, '1...', id)
            tobz.getGroupMembers(groupId)
            tobz.removeParticipant(groupId, author)
        }
        ...
        case '#linknagroup':
        case '#yonigroup':
            tobz.reply(from, `Link Group  :\nName Group : UzY Patcher  🥀\nhttps://chat.whatsapp.com/Khy0aaAVmkX3rTrCQ0pjcy\nJangan Lupa Join Ya Kak ${pushname}`, id)
            break
        case '#groupmenu':
        ...
        case '#iklan':
            ...
            break
        case '#adminmenu':
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin yoni', id)
        ...
        case 'hai':
        case 'halo':
            if (!isHiMsg) return tobz.reply(from, `Hi too @${pushname}, >//<`, id)
            break
        case 'assalamualaikum':
            if (!isHiMsg) return tobz.reply(from, `Waalaikumsaalam @${pushname}, >//<`, id)
            break
        case 'bot':
        case 'Bot':
            if (!isHiMsg) return tobz.reply(from, `Ya Sayang? Ketik, *#help* untuk bantuan`, id)
            break
        default:
            if (!isGroupMsg) return tobz.reply(from, 'Jika Ingin Menggunakan Bot Harap Masuk Ke Dalam Grup yoni, Link Ada Di Bio atau Bisa Mengetik #yonigroup!\nJika Ingin Sewa Bot atau Bikin Bot Harap Ketik *#iklan*', id)
